(function (global) {
	...
	renderHtml: function (data, context) {
		...
		if (parent.basicType() == "object") {
            ...
            if (data.basicType() == "object" && data.value().args) {
                showDelete = false;   // don't show delete icon for elements
                // Crude, should be smarter about this.
            }
	    var result = "";
	    if (showDelete) {
	...
	renderHtml: function (data, context) {
		...
		if (subData.defined()) {
                    console.log(key, subData.schemas().fixed().definedProperties());
		    var title = subData.schemas().fixed().title();
                    var subType = subData.schemas().fixed()[0].data.value().type;
		} else {
		    ...
		    result += '<tr class="json-object-pair">';
                    result += '<td>';
                    if (key != "args") {
		        result += '<tr><td colspan="2" class="json-object-key"><div class="json-object-key-title">' + escapeHtml(key) + '</div><div class="json-object-key-text">' + escapeHtml(title) + '</div></td></tr>';
		        result += '<tr><td colspan="2" class="json-object-value">';
                    }
